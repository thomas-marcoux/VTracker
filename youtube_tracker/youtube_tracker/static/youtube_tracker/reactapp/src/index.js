...
const APP = observer(() => {
  ...
  useEffect(() => {
    console.log("Getting started with tracxker Id ", trackerData.tracker_id, trackerData);
    ...
    fetch(`${window.location.host.match('8600') ? 'http://127.0.0.1:8000' : ''}/chartData`, {
      method: 'POST',
      headers: {
      ...
      .then((data) => {
        ...
        fileDataStore.init(file, configStore.uiStyle);
    ...
    webworkerStore.addWorkerEventListener(d => {
      ...
      const { type, data } = d;
      console.log("The type ", type, data);
      switch (type) {
        ...
        case 'end parse vosviewer-map-network file': {
          ...
          if (!fileDataStore.fileError) {
            ...
            console.log("The operation successful", fileDataStore);
